function [final] = Control_sledzenie(t,q)
x = q(1);
y = q(2);
teta = q(3);
z_d1 = q(4);
...
q_r = [(1/(1+trP1*t))*cos(trP2*t)-1;(1/(1+trP1*t))*sin(trP2*t);teta_r];
%q_r = [(1+.2*sin(.9*t)) * cos(.15*t) - 1 ; (1+.2*sin(.9*t)) * sin(.15*t);teta_r];

dqxy = [- (trP1*cos(t*trP2))/(t*trP1 + 1)^2 - (trP2*sin(t*trP2))/(t*trP1 + 1);... 
    (trP2*cos(t*trP2))/(t*trP1 + 1) - (trP1*sin(t*trP2))/(t*trP1 + 1)^2];
%dqxy = [(9*cos((9*t)/10)*cos((3*t)/20))/50 - (3*sin((3*t)/20)*(sin((9*t)/10)/5 + 1))/20;...
%(3*cos((3*t)/20)*(sin((9*t)/10)/5 + 1))/20 + (9*cos((9*t)/10)*sin((3*t)/20))/50];

d2qxy =[(2*trP1^2*cos(t*trP2))/(t*trP1 + 1)^3 - (trP2^2*cos(t*trP2))/(t*trP1 + 1) + (2*trP1*trP2*sin(t*trP2))/(t*trP1 + 1)^2;...
(2*trP1^2*sin(t*trP2))/(t*trP1 + 1)^3 - (trP2^2*sin(t*trP2))/(t*trP1 + 1) - (2*trP1*trP2*cos(t*trP2))/(t*trP1 + 1)^2];
%d2qxy =[- (9*cos((3*t)/20)*(sin((9*t)/10)/5 + 1))/400 - (27*cos((9*t)/10)*sin((3*t)/20))/500 - (81*cos((3*t)/20)*sin((9*t)/10))/500;...
%  (27*cos((9*t)/10)*cos((3*t)/20))/500 - (9*sin((3*t)/20)*(sin((9*t)/10)/5 + 1))/400 - (81*sin((9*t)/10)*sin((3*t)/20))/500];
 
%q_r = [sin(t);cos(t);teta_r];
%dqxy=[cos(t);-sin(t)];                                          
...
eta_r = inv(S_2_2_r)*dqxy; %[q_r(1);q_r(2)];
...
%eta_r_k = S_2_2_k_r*[q1_r(1);q1_r(2)]*q1_r(3) + inv(S_2_2_r)*[q1_r_k(1);q1_r_k(2)];
%q_r_k=[- cos(t/5)/(50*(t/50 + 1)^2) - sin(t/5)/(5*(t/50 + 1));cos(t/5)/(5*(t/50 + 1)) - sin(t/5)/(50*(t/50 + 1)^2);eta_r(2)];

q_r_k=[dqxy;eta_r(2)];


%q_r_kk_xy=[(2*cos(t/5))/(25*(t/5 + 1)^3) - cos(t/5)/(25*(t/5 + 1)) + (2*sin(t/5))/(25*(t/5 + 1)^2);(2*sin(t/5))/(25*(t/5 + 1)^3) - sin(t/5)/(25*(t/5 + 1)) - (2*cos(t/5))/(25*(t/5 + 1)^2)];

q_r_kk_xy= d2qxy;

eta_r_k = S_2_2_k_r*[q_r_k(1);q_r_k(2)]*eta_r(2) + inv(S_2_2_r)*[q_r_kk_xy(1);q_r_kk_xy(2)];


q_r_kk=[(2*cos(t/5))/(25*(t/5 + 1)^3) - cos(t/5)/(25*(t/5 + 1)) + (2*sin(t/5))/(25*(t/5 + 1)^2);(2*sin(t/5))/(25*(t/5 + 1)^3) - sin(t/5)/(25*(t/5 + 1)) - (2*cos(t/5))/(25*(t/5 + 1)^2);eta_r_k(2)];
%q_r_kk=[-sin(t);-cos(t);eta_r_k(2)];
...
Omega1_k = (delta_d_2k*delta_d - delta_d_k^2)/(delta_d^2) + k1*((2*w*w_k*delta_d^2 - w^2*2*delta_d*delta_d_k)/(delta_d^4)) + (w*f_k * delta_d^2 + w_k*f * delta_d^2 - w*f * 2*delta_d*delta_d_k)/(delta_d^4); 
...
Ap = [0, q1(3) ; -q1(3),0];
Y1 = T.'*([1,0;0,x_ICR^2]*(T*u_d_k + T_k*u_d + II_k) + x_ICR*Ap*(T*u_d+II));
...
Y4 = T.'*[-sin(teta) * SUMy ; x_ICR*cos(teta)*SUMy-a*((g*sgn(v1y))+(g*sgn(v4y))) + b*((g*sgn(v2y))+(g*sgn(v3y)))];
Y_d = [Y1,Y2,Y3,Y4];

tau_a = 0*Y_d * (ro^2*Y_d.'*u_f)/(norm(Y_d.'*u_f)*ro+e2);
z_f = z_d - z;
...
if mod(iteratorDisp, 10000000)
    t;
    q_f.';
end

global tau_vector t_vector;

tau_vector(iteratorDisp+1,:) = tau;
t_vector(iteratorDisp+1) = t;
iteratorDisp = iteratorDisp + 1;

final = [q1;z_d_k;u_k;eta_r(2);q_f_k;dqxy];